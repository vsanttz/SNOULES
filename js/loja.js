// js/loja.js - VERSAO COMPLETA COM GALERIA E PAINEL ADMIN
...
const AppState = {
    ...
    // ===== FUNÇÕES PARA SALVAR PEDIDOS NO PAINEL ADMIN =====
    ...
    completeOrder() {
        ...
        const orderData = {
            ...
            items: [...this.cart], // Fazer cópia do carrinho atual
        ...
        // Salvar pedido - USANDO A NOVA FUNÇÃO
        const success = this.saveOrderToAdmin(orderData);
    ...
    // NOVA FUNÇÃO PARA SALVAR PEDIDOS NO ADMIN
    saveOrderToAdmin(orderData) {
        try {
            console.log('💾 Salvando pedido no admin...', orderData.id);
            
            // Carregar pedidos existentes do localStorage
            const existingOrders = JSON.parse(localStorage.getItem('snoules-orders') || '[]');
            
            console.log('📦 Pedidos existentes:', existingOrders.length);
            ...
            if (!orderExists) {
                ...
                console.log('✅ Pedido salvo com sucesso no admin!');
                console.log('🆔 ID:', orderData.id);
                console.log('👤 Cliente:', orderData.customer.fullName);
                console.log('💰 Total: R$', orderData.total.toFixed(2));
                console.log('📊 Total de pedidos agora:', existingOrders.length);
                
                return true;
            } else {
                console.log('⚠️ Pedido já existe, não será duplicado');
                return true;
            }
        } catch (error) {
            console.error('❌ Erro ao salvar pedido no admin:', error);
            return false;
        }
    },
